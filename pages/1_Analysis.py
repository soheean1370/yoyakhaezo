...
st.set_page_config(page_title="약관 분석 | 요약해조", page_icon="📄", layout="wide", initial_sidebar_state="collapsed")
...
.sec-title { font-size:1.05rem; font-weight:700; color:#444; margin:1.5rem 0 0.8rem 0; }
</style>
...
if not uploaded:
    ...
    if st.button("🔍 약관 분석 시작하기"):
        ...
        try:
            ...
            with st.spinner("🧠 AI가 약관을 분석하고 있어요... (약 10~20초)"):
    ...
    # 결과 표시 (session_state에 있으면 항상 보여줌)
    if "result" in st.session_state:
        ...
        with c1:
            st.markdown(f'<div class="stat-box"><div class="stat-val" style="color:#F5B800">{len(text):,}</div><div class="stat-lbl">📄 추출된 글자 수</div></div>', unsafe_allow_html=True)
        with c2:
            st.markdown(f'<div class="stat-box"><div class="stat-val" style="color:#00BFA5">{len(result["benefits"])}</div><div class="stat-lbl">✅ 혜택 항목</div></div>', unsafe_allow_html=True)
        with c3:
            st.markdown(f'<div class="stat-box"><div class="stat-val" style="color:#FF7043">{len(result["cautions"])}</div><div class="stat-lbl">⚠️ 주의사항 항목</div></div>', unsafe_allow_html=True)

        st.markdown("<br>", unsafe_allow_html=True)

        st.markdown('<div class="sec-title">🗒️ 핵심 요약</div>', unsafe_allow_html=True)
        st.markdown(f"""
        <div class="summary-card">
            <div style="font-size:0.75rem;font-weight:700;letter-spacing:2px;color:#F5B800;margin-bottom:0.5rem">✨ SUMMARY</div>
            <div style="color:#444;font-size:1.05rem;line-height:1.8">{result['summary']}</div>
        </div>""", unsafe_allow_html=True)

        c1, c2, c3 = st.columns(3)
        with c1:
            st.markdown('<div class="sec-title">✅ 혜택</div>', unsafe_allow_html=True)
            items = "".join([f'<div class="item-row"><span style="color:#2E7D32">●</span>{b}</div>' for b in result["benefits"]]) or "해당 없음"
            st.markdown(f'<div class="card" style="border-top:4px solid #A5D6A7">{items}</div>', unsafe_allow_html=True)

        with c2:
            st.markdown('<div class="sec-title">⚠️ 주의사항</div>', unsafe_allow_html=True)
            items = "".join([f'<div class="item-row"><span style="color:#E65100">●</span>{c}</div>' for c in result["cautions"]]) or "해당 없음"
            st.markdown(f'<div class="card" style="border-top:4px solid #FFCC80">{items}</div>', unsafe_allow_html=True)

        with c3:
            st.markdown('<div class="sec-title">📌 조건</div>', unsafe_allow_html=True)
            items = "".join([f'<div class="item-row"><span style="color:#1565C0">●</span>{c}</div>' for c in result["conditions"]]) or "해당 없음"
            st.markdown(f'<div class="card" style="border-top:4px solid #90CAF9">{items}</div>', unsafe_allow_html=True)

        if result["guide"]:
            st.markdown('<div class="sec-title">👉 꼭 해야 할 것</div>', unsafe_allow_html=True)
            items = "".join([f'<div class="item-row"><span style="color:#00BFA5;font-weight:700">→</span>{g}</div>' for g in result["guide"]])
