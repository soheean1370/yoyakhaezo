...
if "result" not in st.session_state:
    ...
    st.session_state["result"] = {
        ...
        "guide": [
            "전월 실적 조건을 본인의 소비 패턴으로 충족할 수 있는지 확인하세요.",
            "자주 사용하는 가맹점이 혜택 대상에 포함되는지 확인하세요.",
            "할인 한도와 제외 조건을 확인한 뒤 가입 여부를 결정하세요.",
            "연회비 대비 실제로 받을 수 있는 혜택이 더 큰지 비교해보세요.",
        ],
    }
...
def get_default_guide(category):
    """
    AI 결과에 guide가 없을 때 문서 유형별 기본 행동 가이드를 제공하는 함수
    """
    if category == "card":
        return [
